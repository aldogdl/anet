...
class NG2ContactosRepository extends ServiceEntityRepository implements PasswordUpgraderInterface
{
  ...
  /**
   * Used to upgrade (rehash) the user's password automatically over time.
   * $2y$13$3vdApQIgTWWoj5VPqyu0pef0.ZLXFDhWS3iK8m9eKTuN3LIfMZmry
   */
  public function cambiarPassword(int $idCot, string $newPassword): array
  {
    ...
    if($user) {
      $user = $user[0];
      file_put_contents('err_pass_1.txt', '');
      $pass = $this->encodePassword($user, $newPassword);
      file_put_contents('err_pass_2.txt', '');
      $user->setPassword('1234567');
      $user->setNombre($pass);
      $this->_em->persist($user);
      $this->_em->flush();
      file_put_contents('err_pass_3.txt', '');
      return [
        'id' => $user->getId(),
