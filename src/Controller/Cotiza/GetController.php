...
class GetController extends AbstractController
{
    ...
    #[Route('cotiza/get-status-ordenes/{version}/', methods:['get'])]
    public function getStatusOrdenes(StatusRutas $ruta, $version): Response
    {   
        return $this->json([
            'abort'=>false, 'msg' => 'ok',
            'body' => $ruta->getLastRuta($version)
        ]);
    }
    ...
    ): Response
    {
        ...
        if($orden) {
            ...
            $stts = $rutas->getRutaByFilename($orden->getRuta());
            ...
            $rota = count($idsPzas);
            $data = $centinela->getSchemaInit($sttPza, $orden->getRuta());
            for ($i=0; $i < $rota; $i++) { 
            ...
            if($saved) {
                ...
                $result = ['abort' => !$saved, 'body' => $data['stt']];
            }
        }
