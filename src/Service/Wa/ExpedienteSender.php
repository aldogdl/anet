...
class ExpedienteSender
{
    ...
    public function processAndSend(
			...
			$isSuccess = ($totalFailed === 0 && $totalSent > 0);

			// Si los mensajes se enviaron con éxito (status 200 OK), eliminamos el archivo del expediente
			if ($totalSent > 0 && file_exists($filePath)) {
				@unlink($filePath);
			}
			...
			return [
				...
				'deleted'     => ($totalSent > 0 && !file_exists($filePath)),
