...
class Ordenes
{
    ...
    public function toArray()
    {
        ...
        $piezas = [];
        if($this->avo) {
            $avoTo = $this->avo->toArray();
        }
        $rota = count($this->piezas);
        for ($i=0; $i < $rota; $i++) { 
            $piezas[] = $this->piezas[$i]->toArray();
        }
        return [
            'est' => $this->est,
            'stt' => $this->stt,
            'id' => $this->id,
            'own' => $this->own->toArray(),
            'marca' => [
                'id' => $this->marca->getId(),
                'nombre' => $this->marca->getNombre(),
                'simyls' => $this->marca->getSimyls()
            ],
            'modelo' => [
                'id' => $this->modelo->getId(),
                'nombre' => $this->modelo->getNombre(),
                'simyls' => $this->modelo->getSimyls()
            ],
            'anio' => $this->anio,
            'isNac' => $this->isNac,
